...
from .utils import sliding_window, text as text_utils
...
async def generate(
    ...
    document_text = text_utils.extract_document(request.input)
    user_text = text_utils.strip_document_tags(request.input)
    resume_text = document_text or user_text
    ...
    try:
        intent = intent_tools.intent_router(user_text or request.input)
    except Exception as exc:  # noqa: BLE001
    ...
    search_results = await _maybe_search(user_text, request.web_search)

    chunk_ids: List[str] = []
    bm25_extra: List[Dict[str, str]] = []

    if document_text:
        chunks = text_utils.chunk_text(document_text)
        metadatas = [
            {
                "source": f"user_doc#{idx}",
                "type": "document",
    ...
    agent_state = {
        "intent": intent,
        "history": history_str,
        "user_input": user_text or request.input,
    ...
    try:
        ...
        if chunk_ids and not request.persist_documents:
    ...
    user_entry = History(role="user", content=request.input, intent=intent)
